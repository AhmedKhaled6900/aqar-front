...
export function AdminAttributesPage() {
  ...
  const canLink = hasPermission('attribute.update')
  ...
  const handleDelete = (id: string) => {
    confirm({
      description: t('attributes.deleteConfirm'),
      onConfirm: async () => {
        setError('')
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t('attributes.catalogDesc')}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {canLink && (
            <Button variant="outline" asChild>
              <Link to="/admin/attributes/links">
                <Link2 className="h-4 w-4" />
                {t('attributes.linkToSubcategories')}
              </Link>
            </Button>
          )}
          {canCreate && (
